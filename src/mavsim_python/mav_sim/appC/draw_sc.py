...
class DrawSc:
    """Object for drawing the spacecraft"""

    def __init__(self, state: MsgState, window: gl.GLViewWidget) -> None:
        """
        Draw the Spacecraft.
        ...
        self.sc_points, self.sc_meshColors = get_points()

        sc_position = np.array(
        ...
        rotated_points = rotate_points(self.sc_points, R)
        translated_points = translate_points(rotated_points, sc_position)

        # convert points to triangular mesh defined as array of three 3D points (Nx3x3)
        mesh = points_to_mesh(translated_points)
        self.sc_body = gl.GLMeshItem(
...
def rotate_points(points: NP_MAT, R: NP_MAT) -> NP_MAT:
    "Rotate points by the rotation matrix R"
    rotated_points = R @ points
    return rotated_points


def translate_points(points: NP_MAT, translation: NP_MAT) -> NP_MAT:
    "Translate points by the vector translation"
    translated_points = points + np.dot(translation, np.ones([1, points.shape[1]]))
    return cast(NP_MAT, translated_points)


def get_points() -> tuple[NP_MAT, NP_MAT]:
    """ "
...
def points_to_mesh(points: NP_MAT) -> NP_MAT:
    """ "
    ...
    mesh = np.array(
        [
            [points[0], points[1], points[5]],  # front 1
