...
from mav_sim.state_plotter.plotter_args import PlotArgs
from mav_sim.state_plotter.state_data import StateData


class StatePlot:
    """Plotting of the state"""

    def __init__(self, plotbox: pg.graphicsItems.PlotItem, args: PlotArgs) -> None:
        """
        Initialize the plot
        """
        ...
        if self.has_sigma and self.sigma_bounds is not None:
            ...
            for bound in self.sigma_bounds:
                ...
                if self.dimension == 1:
                    lower_plot = self.plotbox.plot(
                        name="{} {}-sigma error bound".format(self.name, bound)
                    )
                    upper_plot = self.plotbox.plot()
                    self.sigma_plots[bound] = {
                        "lower_plot": lower_plot,
                        "upper_plot": upper_plot,
                        "pen": dashed_pen,
                    }
                else:
                    sigma_plot = self.plotbox.plot(
                        name="{} {}-sigma error bound".format(self.name, bound)
                    )
                    self.sigma_plots[bound] = {"plot": sigma_plot, "pen": dashed_pen}
        self.marker = None
        self.marker_scale = (
            0.04  # Percentage of the minimum plotbox dimension for the circle radius
        )
        if self.dimension == 2:
            self.marker = plotbox.plot()
            self.xy_marker_circle = get_ellipse((0, 0), radius=1.0)

        # Initialize states
        self.states: dict[str, StateData] = OrderedDict()

        for name in args.state_names:
            self.add_state(
                name, self.sigma_bounds, args.max_length, args.is_angle, args.rad2deg
            )

    def add_state(
        self,
        name: str,
        sigma_bounds: Optional[list[int]] = None,
        max_length: Optional[int] = None,
        is_angle: bool = False,
        rad2deg: bool = False,
    ) -> None:
        """Creates a new state data object for storing data"""
    ...
    def get_states(self) -> dict[str, StateData]:
        """Returns the state data dictionary"""
        return self.states

    def update(self) -> None:
        """Appears to update the sigma value (uncertainty) and then plot the data"""
        if self.hidden:
            return
        ...
        if self.dimension == 1:
            ...
            if self.has_sigma and self.sigma_bounds is not None:
                for bound in self.sigma_bounds:
                    ...
                    self.sigma_plots[bound]["lower_plot"].setData(
                        x_data, state_objs[0].sigma_data[bound]["lower"], pen=pen
                    )
                    self.sigma_plots[bound]["upper_plot"].setData(
                        x_data, state_objs[0].sigma_data[bound]["upper"], pen=pen
                    )
            ...
            if self.has_sigma and self.sigma_bounds is not None:
                for bound in self.sigma_bounds:
                    ...
                    ellipse = get_ellipse(center=center, radius=radius)
                    self.sigma_plots[bound]["plot"].setData(
                        ellipse[0], ellipse[1], pen=pen
                    )
        else:
        ...
        if not self.connect:
            ...
            if self.marker is not None and len(x_data) > 0 and len(y_data) > 0:
                ...
                scale = self.marker_scale * min(
                    x_range[1] - x_range[0], y_range[1] - y_range[0]
                )
                marker = scale * self.xy_marker_circle + np.array(
                    [[x_data[-1]], [y_data[-1]]]
                )
...
def get_ellipse(
    center: Union[list[int], list[float], tuple[int, int], tuple[float, float]],
    radius: Union[int, float, list[int], list[float]],
) -> npt.NDArray[Any]:
    """Calculates points along an ellipse (or circle if the radius is scalar)"""
    N = 100
