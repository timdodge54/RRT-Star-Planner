...
from mav_sim.state_plotter.plotter_args import *
...
theta_plots = PlotboxArgs(title="Multiple theta plots", plots=["theta", "theta_e"])
...
xy_plot = PlotboxArgs(
    title="XY Plane",
    plots=[PlotArgs(states=["x", "y"]), PlotArgs(states=["x_truth", "y_truth"])],
)
# -Add names to the different 2D curves
xz_plot = PlotboxArgs(
    title="XZ plane",
    plots=[
        PlotArgs("Estimated xz position", states=["x", "z"]),
...
yz_plot = PlotboxArgs(
    title="YZ plane",
    plots=[
        PlotArgs("Estimated yz position", states=["y", "z"]),
...
plots: list[Union[list[PlotboxArgs], list[str]]] = [first_row, second_row, third_row]
