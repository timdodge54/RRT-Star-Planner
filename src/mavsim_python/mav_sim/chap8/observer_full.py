...
class ekfFullState:
    ...
    def __init__(self) -> None:
        ...
        self.R_analog = np.diag(
            [SENSOR.abs_pres_sigma ** 2, SENSOR.diff_pres_sigma ** 2, 0.00001 ** 2]
        )
        self.R_gps = np.diag(
            [
    ...
    def update(self, measurement: MsgSensors) -> MsgState:
        ...
        wind_world = np.array(
            [[self.estimated_state.wn], [self.estimated_state.we], [0]]
        )
    ...
    def propagate_model(self, measurement: MsgSensors) -> None:
        ...
        for _ in range(0, self.N):
            ...
            Gg_d = np.concatenate(
                (np.zeros((3, 3)), -vel_cross, -S, np.zeros((5, 3))), axis=0
            )
            Ga_d = np.concatenate(
                (np.zeros((3, 3)), -np.eye(3), np.zeros((8, 3))), axis=0
            )
            # update P with discrete time model
            self.P = A_d @ self.P @ A_d.T + self.Ts ** 2 * (
                self.Q + Gg_d @ self.Q_gyro @ Gg_d.T + Ga_d @ self.Q_accel @ Ga_d.T
            )
    ...
    def measurement_update(self, measurement: MsgSensors) -> None:
        ...
        y = np.array([[measurement.abs_pressure, measurement.diff_pressure, 0.0]]).T
    ...
    def h_analog(
        self, x: npt.NDArray[Any], measurement: MsgSensors
...
@no_type_check
def jacobian(
    fun: JacobianFnc, x: npt.NDArray[Any], measurement: MsgSensors
) -> npt.NDArray[Any]:
    """
